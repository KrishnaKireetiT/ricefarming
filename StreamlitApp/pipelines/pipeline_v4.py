"""
Multi-Modal Rice Farming KG Pipeline V4
Extracted from kg_pipeline_multimodal_v4(fixed_langfuse_agent_graph)).ipynb

This module provides the pipeline implementation for the Streamlit testing app.
"""
...
# ================================================================
# Pipeline V4 Implementation
# ================================================================

class PipelineV4(BasePipeline):
    """
    Multi-Modal Rice Farming KG Pipeline V4
    ...
    def get_name(self) -> str:
        return "Multi-Modal KG Pipeline V4"
    
    def get_version(self) -> str:
        return "4.0.0"
    
    def get_description(self) -> str:
        return "Multi-modal rice farming pipeline with L1/L2 KG, visual nodes, hybrid search, and Langfuse agent graph tracing."
    ...
    def initialize(self) -> None:
        ...
        logger.info("Initializing Pipeline V4...")
        ...
        logger.info("Pipeline V4 initialized successfully.")
        ...
        if self._entity_cache is None:
            kg_entities = self.graph.query("""
                MATCH (n) WHERE n.name IS NOT NULL
                  AND NOT n:Chunk AND NOT n:Section AND NOT n:Chapter AND NOT n:Document AND NOT n:Visual
    ...
    def verify_database(self) -> dict:
        ...
        try:
            ...
            visual_result = self.graph.query("MATCH (v:Visual) RETURN count(v) as count")
            status["visual_count"] = visual_result[0]["count"] if visual_result else 0
            ...
            required_vector = [f"{config.VECTOR_INDEX_NAME}_chunk", f"{config.VECTOR_INDEX_NAME}_visual"]
    ...
    def _vector_search_chunks_and_visuals(self, query: str, k: int = None) -> List[Dict]:
        """Semantic search over Chunks and Visuals."""
        if k is None:
        ...
        try:
            chunk_results = self.graph.query("""
                ...
                RETURN 'Chunk' AS type, node.chunk_id AS id, node.embedding_text AS text, 
                       node.title AS title, score, null AS image_path
                ORDER BY score DESC
            ...
            try:
                fallback = self.graph.query("""
                    ...
                    RETURN 'Chunk' AS type, c.chunk_id AS id, c.embedding_text AS text,
                           c.title AS title, 0.5 AS score, null AS image_path
                    LIMIT $k
                """, {"k": k})
        ...
        # Try visual search
        try:
            visual_results = self.graph.query("""
                CALL db.index.vector.queryNodes($index_name, $k, $embedding)
                YIELD node, score
                RETURN 'Visual' AS type, node.visual_id AS id, node.embedding_text AS text,
                       node.caption_en AS title, score, node.image_path AS image_path
                ORDER BY score DESC
            """, {"index_name": f"{config.VECTOR_INDEX_NAME}_visual", "k": k, "embedding": query_embedding})
            results.extend(list(visual_results))
        except Exception as e:
            logger.warning(f"Visual vector search failed: {e}")
    ...
    def _keyword_search_chunks(self, query: str, k: int = None) -> List[Dict]:
        ...
        try:
            ...
            if words:
                # Create OR query with wildcards for partial matching
        ...
        try:
            # Extract key words from query (simple tokenization)
            words = [w.strip() for w in query.split() if len(w.strip()) > 2]
            if not words:
                return []
            
            # Use first significant word for CONTAINS
    ...
    def _build_farmer_answer(self, evidence: dict) -> str:
        ...
        visual_refs = evidence.get("visual_refs", [])[:3]
        visual_text = "\n".join([f"[VISUAL-{i+1}] {v[0]} -> {v[1]}" for i, v in enumerate(visual_refs)])
        
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a friendly rice farming advisor. Answer using ONLY the context provided.
- Use simple language
- Cite sources as [DOC-N]
- Reference visuals by their path
...
{visuals}""")
        ...
        result = chain.invoke({"question": evidence["question"], "context": context_text, "visuals": visual_text})
    ...
    def run_query(self, question: str) -> PipelineResult:
        """
        Run the full multi-modal rice farming advisor pipeline.
        ...
        with self.langfuse.start_as_current_observation(
            ...
            vector_results = self._vector_search_chunks_and_visuals(question)
            ...
            tool2.update(output={
                ...
                "chunks": len([r for r in vector_results if r["type"] == "Chunk"]),
                "visuals": len([r for r in vector_results if r["type"] == "Visual"])
            })
            tool2.end()
            ...
            with self.langfuse.start_as_current_observation(
                ...
                visual_refs = [(r["title"], r["image_path"]) for r in merged if r.get("image_path")]
                agg.update(output={"merged_count": len(merged), "visual_refs_count": len(visual_refs)})
            
            result["vector_context"] = merged
            result["visual_refs"] = visual_refs
